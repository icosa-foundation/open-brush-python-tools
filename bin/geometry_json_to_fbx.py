...
if not found:
    print(
        "Please install the Python FBX SDK: http://www.autodesk.com/products/fbx/",
        file=sys.stderr,
    )
...
def as_fvec4(tup, scale=1):
    if len(tup) == 3:
        ...
        return FbxVector4(
            tup[0] * scale, tup[1] * scale, tup[2] * scale, tup[3] * scale
        )
...
def create_fbx_layer(
    fbx_mesh, data, converter_fn, layer_class, allow_index=False, allow_allsame=False
):
    ...
    elif allow_index and len(unique_data) <= len(data) * 0.7:
...
def add_mesh_to_scene(sdk, scene, mesh):
    ...
    layer_elt = create_fbx_layer(fbx_mesh, mesh.n, as_fvec4, FbxLayerElementNormal)
    ...
    layer_elt = create_fbx_layer(
        fbx_mesh, mesh.uv0, as_fvec2, FbxLayerElementUV, allow_index=True
    )
    if layer_elt is not None:
    ...
    layer_elt = create_fbx_layer(
        fbx_mesh, mesh.t, as_fvec4, FbxLayerElementTangent, allow_index=True
    )
    if layer_elt is not None:
    ...
    layer_elt = create_fbx_layer(
        fbx_mesh, ((0, 0, 0, 0),), as_fvec4, FbxLayerElementBinormal, allow_allsame=True
    )
    if layer_elt is not None:
...
def main():
    ...
    grp.add_argument(
        "--weld-verts", action="store_true", help="(default) Weld vertices"
    )
    grp.add_argument(
        "--no-weld-verts",
        action="store_false",
