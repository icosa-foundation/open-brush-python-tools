...
def _quaternion_multiply_vector(q, v):
    qv = v + [0]
    return _quaternion_multiply_quaternion(
        _quaternion_multiply_quaternion(q, qv), _quaternion_conjugate(q)
    )[:3]
...
def _adjust_guide(scene_translation, scene_rotation, scene_scale, guide):
    ...
    _adjust_transform(
        scene_translation, scene_rotation, scene_scale, guide["Transform"]
    )
...
def _adjust_transform(scene_translation, scene_rotation, scene_scale, transform):
    ...
    translatedTranslation = [
        b + a for a, b in zip(scene_translation, rotatedTranslation)
    ]
...
def normalize_tilt_file(tilt_file):
    ...
    for stroke in tilt_file.sketch.strokes:
        ...
        for cp in stroke.controlpoints:
            ...
            cp.position = pos
            cp.orientation = _quaternion_multiply_quaternion(
                scene_rotation, cp.orientation
            )

    with tilt_file.mutable_metadata() as metadata:
        # Reset scene transform to be identity.
        metadata["SceneTransformInRoomSpace"][0] = [0.0, 0.0, 0.0]
        metadata["SceneTransformInRoomSpace"][1] = [0.0, 0.0, 0.0, 1.0]
        metadata["SceneTransformInRoomSpace"][2] = 1.0
        ...
        if "ModelIndex" in metadata:
            for model_type in metadata["ModelIndex"]:
                for transform in model_type["Transforms"]:
                    _adjust_transform(
                        scene_translation, scene_rotation, scene_scale, transform
                    )
        ...
        if "Lights" in metadata:
            metadata["Lights"]["Shadow"]["Orientation"] = (
                _quaternion_multiply_quaternion(
                    scene_rotation, metadata["Lights"]["Shadow"]["Orientation"]
                )
            )
            metadata["Lights"]["NoShadow"]["Orientation"] = (
                _quaternion_multiply_quaternion(
                    scene_rotation, metadata["Lights"]["NoShadow"]["Orientation"]
                )
            )
        ...
        if "Environment" in metadata:
            ...
            metadata["Environment"]["GradientSkew"] = _quaternion_multiply_quaternion(
                scene_rotation, metadata["Environment"]["GradientSkew"]
            )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Create a normalized version of the sketch (with 'Normalized' appended to\
