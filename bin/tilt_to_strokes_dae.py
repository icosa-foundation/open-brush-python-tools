...
class ColladaFile(object):
                ...
                children=[
                    Element(
                        ...
                        children=[
                            Element(
                                "authoring_tool",
                                text="Tilt Brush COLLADA stroke converter",
                            )
                        ],
                    ),
    ...
    def _init_material(self):
        effect = ET.SubElement(
            self.library_effects, "effect", id=self.make_id("effect_")
        )
        effect.append(
            Element(
                "profile_COMMON",
                        ...
                        children=[
                            Element(
                                ...
                                children=[
                                    ...
                                    Element(
                                        "shininess",
                                        children=[Element("float", text="0.5")],
                                    ),
                                ],
                            )
                        ],
        ...
        material = ET.SubElement(
            self.library_materials, "material", id=self.make_id("material_"), name="Mat"
        )
    ...
    def _init_scene(self):
        visual_scene = ET.SubElement(
            self.library_visual_scenes, "visual_scene", id=self.make_id("scene_")
        )
        self.root.append(
            Element(
                "scene",
                ...
                children=[
                    ...
                    Element(
                        ...
                        children=[
                            ...
                            Element(
                                "p", text=" ".join(map(str, range(len(raw_floats) / 3)))
                            ),
                        ],
                    ),
                ],
...
def main(args):
    ...
    parser = argparse.ArgumentParser(
        description="Converts .tilt files to a Collada .dae containing spline data."
    )
