...
def _read_and_check_header(inf):
    ...
    if len(more_bytes) < more:
        raise ConversionError(
            "Bad header size (claim %s, actual %s)" % (more, len(more_bytes))
        )

    zip_sentinel = inf.read(4)
    if zip_sentinel != "" and zip_sentinel != "PK\x03\x04":
        raise ConversionError(
            "Don't see zip sentinel after header: %r" % (zip_sentinel,)
        )
...
def convert_dir_to_zip(in_name, compress):
    ...
    def by_standard_order(filename):
        ...
        try:
            ...
            raise ConversionError(
                "Unknown file %s; this is probably not a .tilt" % filename
            )
        return (idx, lfile)
    ...
    try:
        ...
        if header_bytes is None:
            ...
            header_bytes = struct.pack(
                HEADER_V1_FMT, "tilT", struct.calcsize(HEADER_V1_FMT), 1, 0, 0
            )
